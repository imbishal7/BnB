...
class Listing(Base):
    ...
    brand = Column(String(100), nullable=True)  # Product brand
    mpn = Column(String(100), nullable=True)  # Manufacturer Part Number
    condition = Column(String(50), nullable=True, default="NEW")  # NEW, USED, REFURBISHED
    ...
    enriched_sku = Column(String(100), nullable=True)  # SKU generated by n8n workflow
    ebay_aspects = Column(JSON, nullable=True)  # eBay product aspects/attributes from n8n
    
    # Status
    status = Column(Enum(ListingStatus), default=ListingStatus.DRAFT, nullable=False)
    error_message = Column(Text, nullable=True)
    
    # eBay publishing details
    ebay_listing_id = Column(String, nullable=True)  # eBay's listing ID
    ebay_offer_id = Column(String, nullable=True)  # eBay's offer ID
    ebay_sku = Column(String, nullable=True)  # SKU used on eBay
    published_at = Column(DateTime, nullable=True)  # When published to eBay
